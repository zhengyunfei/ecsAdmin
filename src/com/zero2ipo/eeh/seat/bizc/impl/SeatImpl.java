...
public class SeatImpl implements ISeatService {
    ...
    public final static String DELETE_BY_MAP="del_"+common+"_ByMap";
    ...
    public boolean deleteByMap(SeatBo bo){
        ...
        try
        {
            queryMap.put("id",bo.getId());
            queryMap.put("seatType",bo.getSeatType());
            baseDao.setDbType(FwConstant.DBTYPE_GLOBAL);
            baseDao.delObject(DELETE_BY_MAP, bo);
            flg=true;
        }catch (Exception e){
            e.printStackTrace();
        }
