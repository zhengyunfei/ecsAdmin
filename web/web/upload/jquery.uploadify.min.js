	...
	var a = {
		...
		onSelect : function(h) {
			...
			for ( var g in this.queueData.files) {
				...
				if (f.uploaded != true && f.name == h.name) {
					var e = confirm('The file named "' + h.name + '" 已经存在了.\n你要替换掉已经存在的吗?');
					if (!e) {
			...
			if (i.itemTemplate == false) {
				i.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">					' +
				'<div class="cancel"><a  href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">移除</a></div>' +
				'<span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>					<div class="uploadify-progress">						<div class="uploadify-progress-bar"><!--Progress Bar--></div>					</div>				</div>';
			}
		...
		onUploadSuccess : function(f, h, d) {
			...
			if (c.inArray("onUploadSuccess", g.overrideEvents) < 0) {
				c("#" + f.id).find(".data").html("\t\t<img class=\"icon403a12\" src=\"../s9/res/img/icon000.png\">");
			}
