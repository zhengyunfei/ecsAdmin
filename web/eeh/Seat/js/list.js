...
function findAllList(url){
	...
	grid = $("#maingrid").ligerGrid({
		...
		checkbox: true,
		columns: [
			{ display: '班级', name: 'name' },
			{ display: '班主任', name: 'teacherName'},
			{ display: '班级人数', name: 'gradenum'},
			{ display: '座位', name: 'seatStatus',render:function(r){
				var h='';
				if(r.seatStatus=='1'){
					h+= "<a href=\"javascript:lookSeat("+ r.id+")\"><font color='green'>座位查看</font></a>";
					h+="<a href=\"javascript:updSet("+r.id+")\"><font color='red'>修改</font></a>";
					return h;
				}else{
...
function add(){
	...
	m = $.ligerDialog.open({ url: url, height: 300,width:500, title:'新增座位'});
}
function upd(id){
	var url="../Course/forUpdateInitPage.shtml?id="+id;
	m = $.ligerDialog.open({ url: url, height: 300,width:500, title:'修改座位',isResize: true ,top:50});
}
function start(id){
...
function lookSeat(id){
	var url="../Seat/forLookSeat.shtml?id="+id+"&type=0";
...
function seatSet(id){
	 ...
	 * @type {string}
	 */
		var url="../Seat/setSeat.shtml?id="+id;
		m = $.ligerDialog.open({ url: url, height: 150,width:300, title:'座位设置'});
}
//座位修改
function updSet(id){
	var url="../Seat/updSeat.shtml?id="+id+"&seatType=0";
	m = $.ligerDialog.open({ url: url, height: 150,width:300, title:'座位修改'});
}
