				...
				{ display: '操作', name: '',render:function(r){
					...
					if(r.status=='已开始'){
						return "<a href='javascript:upd("+r.id+")'>编辑</a>"+
							"&nbsp;&nbsp;<a href='javascript:stop("+r.id+")'>停止</a>";
					}else{
						return "<a href='javascript:upd("+r.id+")'>编辑</a>"+
...
function add(){
	...
	m = $.ligerDialog.open({ url: url, height: 350,width:450, title:'新增培优课程',showMax:true,showToggle:true,showMin:true});
}
function upd(id){
	var url="../Course/forUpdateInitPage.shtml?id="+id;
	m = $.ligerDialog.open({ url: url, height: 350,width:500, title:'修改培优课程',isResize: true ,top:50});
}
function start(id){
