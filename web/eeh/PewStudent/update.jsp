							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								...
								<tr>
									...
									<td width="80%" >
										...
										<select id="className" name="className" style="width:300px">
											<option>请选择</option>
											<c:forEach items="${gradeList}" varStatus="vs" var="grade">
												<option value="${grade.name}" <c:if test="${grade.name==bo.className}">selected="selected" </c:if>>${grade.name}</option>
											</c:forEach>
										</select>
										<span
