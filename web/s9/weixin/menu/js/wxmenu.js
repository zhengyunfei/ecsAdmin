...
function initChildData(url){
 window['g'] = $("#maingrid").ligerGrid({
          ...
          { display: '菜单名称', name: 'codeName', width: '15%' },
          { display: '代码值', name: 'codeValue',  width: '15%',hide:true },
          { display: '菜单标识', name: 'codeType', width: '10%' },
          { display: '显示顺序', name: 'dispSn',  width: '10%' },
          { display: 'content1', name: 'content1',  width: '12%',hide:true },
          { display: '菜单URL', name: 'content2',  width: '12%' },
...
function addCodeInfo(codeSortId,codeType){
  ...
  var url= "../wxMenuManage/forInitSaveCodeInfo.shtml?codeSortId="+codeSortId+"&codeType="+codeType+"&pcode="+pcode+"&codeId="+codeSortId;	
    m=$.ligerDialog.open({ url: url, height: 500,width:550, title:'添加菜单',isResize: true });
  if(!m){
...
function editCodeInfo(){
	...
	var url= "../wxMenuManage/forInitUpdateCodeInfo.shtml?codeId="+codeId;	
...
function delCodeInfo()
{
   var rowid = g.getSelecteds();
	...
	$.ligerDialog.confirm("确定将选中的记录删除吗？", function (yes){
		var rIds="";
		if(yes){
			var url = "../wxMenuManage/delCodeInfo.shtml";
...
function createMenu(){
	...
	$.ajax({
				...
				url : url,
				success : function(data) {
					$.ligerDialog.error("操作成功,需要重新关注才可生效或者24小时内生效！");
