		...
		manager = $("#maingrid").ligerGrid({
	        ...
	        columns: [
	            ...
	            { display: '菜单名称', name: 'menuName', width: 250, align: 'left',render:function (row){
					if(row.menuImg !=""){
						return "<img class='"+row.menuImg+"' src='../res/img/icon000.png'/>&nbsp;<span>"+row.menuName+"</span>";
					}else{
						return row.menuName;
					}
	            }},
	            ...
	            { display: '状态', name: 'isActive', width: 100, type: 'int', align: 'center' ,render: function (row){
					if(row.isActive == "1"){
						return "<img src='../res/img/yes.gif'>";
					}else{
						return "<img src='../res/img/no.gif'>";
					}
		        }},
		...
		$.ajax({
			...
			success:function(data){
				if(data.message == "success"){
					manager.set({url:"/m../menu/stSysMenu"});
...
function start(){
	...
	if(id.length !=0){
		$.ajax({
			...
			success:function(data){
				if(data.message == "success"){
					manager.set({url:"../menu/listSysMenu"});
					$.ligerDialog.success('操作成功!');
				}else{
					$.ligerDialog.error('服务器忙!');
				}
...
function show(){
	var id =  manager.getSelecteds();
	if(id.length !=0){
		var url="../menu/showSysMenu?menuId="+id[0].menuId;
	    $.ligerDialog.open({ title: '系统菜单信息', name:'winselector',width: 700, height: 400, url: url});
	}else{
		$.ligerDialog.error('请选择菜单项!');
	}
 ...
 */
function addSysMenu(){
	var url="sysMenu_add.jsp";
	addManager = $.ligerDialog.open({ title: '新增系统菜单', name:'winselector',width: 800, height: 420, url: url});
 ...
 */
function sysMenuImg(){
	var url="sysMenu_img.html";
	imgManager = $.ligerDialog.open({ title: '系统菜单图片', name:'imgClass',width: 900, height: 600, url: url});
}
