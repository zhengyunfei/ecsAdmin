...
function initChildData(url){
	window['g'] = $("#maingrid").ligerGrid({
		...
		checkbox: true,
		columns: [
			{ display: '代码标识', name: 'codeId', width: '15%',hide:true},
			{ display: '小区名称', name: 'codeName', width: '15%' },
			{ display: '经度', name: 'content1',  width: '12%' },
			{ display: '纬度', name: 'content2',  width: '12%' },
			{ display: '覆盖范围(千米)', name: 'content3',  width: '15%' },
			{ display: '菜单标识', name: 'codeType', width: '10%',hide:true },
...
function addCodeInfo(codeSortId,codeType){
	...
	var url= "../area/forInitSaveCodeInfo.shtml?codeSortId="+codeSortId+"&codeType="+codeType+"&pcode="+pcode+"&codeId="+codeSortId;
	m=$.ligerDialog.open({ url: url, height: 500,width:550, title:'添加菜单',showMax: true, showToggle: true, showMin: true,isResize: true });
	if(!m){
...
function editCodeInfo(){
	...
	var url= "../area/forInitUpdateCodeInfo.shtml?codeId="+codeId;
	m=$.ligerDialog.open({ url: url, height: 480,width:550, title:'编辑代码信息',showMax: true, showToggle: true, showMin: true,isResize: true });
	if(!m){
		$.ligerDialog.error("操作失败！");
		return;
	}
...
function delCodeInfo()
{
	...
	$.ligerDialog.confirm("确定将选中的记录删除吗？", function (yes){
		var rIds="";
		if(yes){
			var url = "../area/delCodeInfo.shtml";
...
function createMenu(){
	var url="../wxmenu/createMenuAjax.shtml";
	$.ajax({
		type : "post",
		url : url,
